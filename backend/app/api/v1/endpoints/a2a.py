...
class A2AHandshakeRequest(BaseModel):
    ...
    public_key: Optional[str] = Field(None, description="Agent's public key for encryption")
...
class A2ANegotiationResponse(BaseModel):
    ...
    proposed_workflow: Dict[str, Any] = Field(..., description="Proposed execution workflow")
...
async def a2a_handshake(request: A2AHandshakeRequest):
    ...
    """
    logger.info("A2A handshake request", agent_id=request.agent_id, agent_name=request.agent_name)
...
async def a2a_negotiate(request: A2ANegotiationRequest):
    ...
    available_skills = [skill for skill in request.requested_skills if skill in our_capabilities]
    ...
    logger.info("A2A negotiation completed", 
                negotiation_id=negotiation_id, 
                accepted=can_handle)
...
async def a2a_communicate(message: A2ACommunicationMessage):
    ...
    """
    logger.info("A2A communication", session_id=message.session_id, message_type=message.message_type)
...
async def terminate_a2a_session(session_id: str):
    ...
    expired_negotiations = [
        nid for nid, negotiation in active_negotiations.items()
