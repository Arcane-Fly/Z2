    ...
    for provider in ProviderType:
        ...
        providers_info[provider.value] = {
            ...
            "capabilities": list(set().union(*[
                model.capabilities for model in provider_models.values()
            ])),
...
async def recommend_optimal_model(
    ...
    for model_id, spec in ALL_MODELS.items():
        if all(cap in spec.capabilities for cap in capabilities):
            if max_cost is None or (
                spec.cost_per_input_token is not None and
                spec.cost_per_input_token <= max_cost
            ):
    ...
    return {
        ...
        "model_info": {
            ...
            "capabilities": [cap.value for cap in suitable_models[recommended].capabilities],
        },
        "alternatives": [
            {
                "model_id": model_id,
