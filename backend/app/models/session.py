...
class MCPSession(Base):
    ...
    client_capabilities: Mapped[dict | None] = mapped_column(JSON)
    server_capabilities: Mapped[dict | None] = mapped_column(JSON)
...
class A2ANegotiation(Base):
    ...
    task_parameters: Mapped[dict | None] = mapped_column(JSON)

    # Negotiation results
    accepted: Mapped[bool] = mapped_column(Boolean)
    proposed_workflow: Mapped[dict | None] = mapped_column(JSON)
    estimated_duration: Mapped[int | None] = mapped_column(Integer)
    cost_estimate: Mapped[float | None] = mapped_column(String(20))  # Using string for decimal precision
...
class TaskExecution(Base):
    ...
    session_id: Mapped[str] = mapped_column(String(255), index=True)
    negotiation_id: Mapped[str | None] = mapped_column(String(255), index=True)
