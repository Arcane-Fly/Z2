...
class ModelRoutingPolicy(Base):
    ...
    # Policy constraints
    max_cost_per_request: Mapped[float | None] = mapped_column(default=None)
    max_latency_ms: Mapped[int | None] = mapped_column(default=None)
    ...
    created_by: Mapped[str | None] = mapped_column(String(100))  # User who created policy
...
class ModelUsageTracking(Base):
    ...
    task_type: Mapped[str | None] = mapped_column(String(50), index=True)
