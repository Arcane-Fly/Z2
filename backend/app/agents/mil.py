...
class DynamicModelRouter:
    ...
    def _filter_by_constraints(
        self, candidates: list[str], policy: RoutingPolicy
    ) -> list[str]:
        ...
        for model_id in candidates:
            ...
            if policy.max_cost_per_request:
                ...
                estimated_tokens = (
                    len("dummy prompt") // 4
                )  # This would be actual request
                estimated_cost = model_info.input_cost_per_million_tokens * (
    ...
    def _normalize_cost_score(self, model_info: ModelInfo) -> float:
        ...
        normalized = 1.0 - (model_info.input_cost_per_million_tokens - min_cost) / (
            max_cost - min_cost
        )
    ...
    def _normalize_latency_score(self, model_info: ModelInfo) -> float:
        ...
        all_latencies = [
            m.avg_latency_ms for m in self.models.values() if m.avg_latency_ms
        ]
        ...
        normalized = 1.0 - (model_info.avg_latency_ms - min_latency) / (
    ...
    async def route_request(
        ...
        if full_model_id in self.performance_history:
            ...
            self.performance_history[full_model_id] = self.performance_history[
                full_model_id
            ][-100:]
