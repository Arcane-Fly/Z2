...
class DynamicPromptGenerator:
    ...
    def _summarize_context(self, context: ContextualMemory) -> str:
        ...
        if context.summary:
            summary_parts.append("Previous: " + str(context.summary.get("main_points", "")))
        
        if context.short_term:
            recent_items = list(context.short_term.items())[-3:]  # Last 3 items
            summary_parts.append("Recent: " + ", ".join([f"{k}: {v}" for k, v in recent_items]))
...
class AdaptiveContextualFlow:
    ...
    def _calculate_recent_success_rate(self, window: int = 5) -> float:
        ...
        successful = sum(1 for interaction in recent_interactions if interaction["success"])
    ...
    def _compress_context(self) -> None:
        ...
        if len(self.interaction_history) > 5:
            ...
            total_tokens = sum(interaction["tokens_used"] for interaction in to_compress)
            success_rate = sum(1 for interaction in to_compress if interaction["success"]) / len(to_compress)
